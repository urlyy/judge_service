...
// -J-Xms128m -J-Xmx512m 
#define COMPILE_CMD "javac %s -encoding UTF8 2>&1"
...
void compile(const char* codeFile,Result *result){
    char command[50];
    sprintf(command, COMPILE_CMD, codeFile);
    // 只返回0和1
...
void _runExe(char *exeFile,long timeLimit, long memoryLimit, char *in, char *out) {
    ...
    if (newstdout != -1 && newstdin != -1){
        ...
        char *cmd[] = {"java","-cp","data", exeFile, NULL};
